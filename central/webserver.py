...
def requires_gh_auth(requested_scope):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            scope = set()
            if (
                req.get_cookie("gh_oauth_token", secret=key) is not None
                and req.get_cookie("gh_oauth_scope", secret=key) is not None
            ):
...
def gh_oauth():
    ...
    token = utils.encrypt_data(
        response["access_token"].encode("ascii"), cfg.github.oauth.client_secret
    )
    scope = utils.encrypt_data(
        response["scope"].encode("ascii"), cfg.github.oauth.client_secret
    )
